...
export const updateTerminal = async (id: string, inputs: Terminal): Promise<Response> => {
    const response = await api.put<Response>(`/api/terminal/${id}`, inputs);
    return response.data;
};

export const deleteTerminal = async (id: string): Promise<Response> => {
...
export const useUpdateTerminal = () => {
    return useMutation({
        mutationFn: async ({ id, terminalData }: { id: string; terminalData: Terminal }) => {
